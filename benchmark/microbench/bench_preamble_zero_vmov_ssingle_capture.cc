...
class BenchPreambleZeroVmovSsingleCapture
  : public EntoBench::CaptureProblem<BenchPreambleZeroVmovSsingleCapture, 132>
{
public:
  void prepare_impl()
  {
...
int main()
{
  ...
  BenchPreambleZeroVmovSsingleCapture problem;
  ENTO_BENCH_HARNESS_TYPE(decltype(problem));
  BenchHarness harness(problem, "bench_preamble_zero_vmov_ssingle_capture");
  harness.run();
